...
An installation script is provided to automate Steps 2–4 and cron job setup:
#+BEGIN_SRC bash
bash install.sh
#+END_SRC
It will create the virtual environment, install dependencies, prompt you for =.env= values, and optionally register the cron job. You can still follow the manual steps below if you prefer.
...
Overall progress: 100%|██████████| 3/3 [00:16<00:00,  5.64s/step]
Successfully wrote board data to data/board.json
Successfully wrote competitions data to data/competitions.json
Successfully wrote workshops data to data/workshops.json
...
│   ├── instructor.py              # Fetch instructor data
│   ├── instructors.py             # Fetch instructor data
│   ├── groups.py                  # Fetch group data
...
├── board_members.json             # Board role-to-member-ID mapping — update this when the board changes
